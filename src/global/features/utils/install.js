...
function toValue(date, converter) {
    ...
    else if (converter === "json") return date.toJSON();
    else if (converter === "timestamp") return date.getTime();
    else return date;
}
...
export const utils = {
    ...
    Enum(enumName, value) {
        ...
        else if (arguments.length === 1) return enumsMap[enumName];
        else if (enumsMap[enumName]) return enumsMap[enumName](value);
        else return "";
    },
    ...
    Convert(value, schema) {
        ...
        if (type === "number" && formatVar === "double")
            // 小数
    ...
    DateDiff(dateTime1, dateTime2, calcType) {
        ...
        const dateTime1Temp = new Date(
            format(new Date(dateTime1), config.formatter)
        ).getTime();
        const dateTime2Temp = new Date(
            format(new Date(dateTime2), config.formatter)
        ).getTime();
